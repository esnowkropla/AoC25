...
-define(FILEPATH, "src/gleam/deque.gleam").
...
-opaque deque(YG) :: {deque, list(YG), list(YG)}.
...
-spec from_list(list(YJ)) -> deque(YJ).
from_list(List) ->
    {deque, [], List}.
...
-spec push_back(deque(YT), YT) -> deque(YT).
...
-spec push_front(deque(YW), YW) -> deque(YW).
...
-spec pop_back(deque(YZ)) -> {ok, {YZ, deque(YZ)}} | {error, nil}.
pop_back(Deque) ->
    case Deque of
        {deque, [], []} ->
            {error, nil};
...
-spec pop_front(deque(AAE)) -> {ok, {AAE, deque(AAE)}} | {error, nil}.
pop_front(Deque) ->
    case Deque of
        {deque, [], []} ->
            {error, nil};
...
-spec reverse(deque(AAJ)) -> deque(AAJ).
reverse(Deque) ->
...
-spec is_logically_equal(deque(AAM), deque(AAM), fun((AAM, AAM) -> boolean())) -> boolean().
...
-spec is_equal(deque(AAU), deque(AAU)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
